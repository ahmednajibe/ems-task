...
const paginationText = computed(() => {
  const start =
    (companiesStore.currentPage - 1) * companiesStore.itemsPerPage + 1;
  const end = Math.min(
...
const handleSave = (companyData: { name: string }) => {
  if (editingCompany.value) {
    ...
    companiesStore.updateCompany(editingCompany.value.id, {
      name: companyData.name,
    });
    toastStore.success(`${companyData.name} updated successfully!`);
  } else {
    // Add new - Server Scripts will calculate counts automatically
  ...
  <DashboardLayout>
    <div class="p-4 space-y-4">
      ...
      <div>
        ...
        <!-- Pagination -->
        <div
          class="flex items-center justify-between px-6 py-4 bg-white border-t border-neutral-200 rounded-b-2xl shadow-soft -mt-2"
        >
